...
`browser-image-scout` is an open source npm package published by NPM-Workbench. Its source code is publicly available for review, audit, contribution, and responsible improvement under the repository license. This package is intended to provide browser-side image-format support detection for client applications and is not designed, published, or maintained for malicious, deceptive, surveillance, credential-harvesting, tracking, or unauthorized activity.
...
The core detection path in this package uses embedded base64 data-URL images and the browser `Image` constructor to determine support in memory. It does not perform outbound HTTP requests on its own for its primary format-detection behavior.
