...
import type { TSupportsPNGReturn } from '../index.js';
...
describe('Supports PNG', () => {
  ...
  test('throws when the window global is not available', async () => {
    const { supportsPNG } = await import('../index.js');

    globalThis.window = undefined as any;
    await expect(supportsPNG()).rejects.toThrow(
  ...
  /* 3 */
  test('resolves "support = true" when PNG is supported', async () => {
    /* setup */
    const { supportsPNG } = await import('../index.js');
    const response: TSupportsPNGReturn = await supportsPNG();

    /* assert */
    expect(response.supported).toBe(true);
    expect(response.mimeType).toBe('image/png');
    expect(response.imgType).toBe('png');
  ...
  /* 4 */
  test('resolves "support = false" when PNG is not supported', async () => {
    /* setup */
    const { supportsPNG } = await import('../index.js');
